...
def FunctionLogic(x, operation, y):
    logger.Log('controller.ClickButton_v2', 'operation_logic.FunctionLogic', [x, operation, y])
    if operation == '+':
        result = functions.PlusFunction(x, y)
        logger.Log('operation_logic.FunctionLogic', 'controller.ClickButton_v2', result)
        return result
    elif operation == '-':
